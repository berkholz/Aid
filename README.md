...
Aid is a tool written in python to download the actual version of specific software packages/products automatically.
...
To get more informations about the components of Aid, take a look at [components](doc/Components.md).
...
The *master branch* is actually on an older state, but with an GUI to activate the specific downloads. Some Crawler modules are outdated, but working properly. Just checkout this branch and try if it fits to your needs.

The *feature_headless* is the actual development that Aid let run headless without a GUI. In a later release the control interface will be in a seperate component.
...
When you run the python pogram all latest found software packages were activated for download. If you don't want that behaviour, then you have to comment out the following line:
```
activate_download_for_latest_found()
```

For a selective activation of downloads you can use the [DB Browser for SQLite](https://sqlitebrowser.org/), open the aid.db and change the value of column download to 1.
