...
def run():
    ...
    for a in tables.find_all('a', href=True):
        ...
        if isBinaryURL(a, 'win64'):
            ...
            downloads.append({"app_platform": "win64", "url_bin": tmp_url_bin, "url_asc": tmp_url_bin + ".asc", "url_sha256": tmp_url_bin + ".sha256"})
        elif isBinaryURL(a, 'android'):
            tmp_url_bin = base_url + findPlatformInURL('android', a['href'])
            app_version = tmp_url_bin.split('-')[1]
            downloads.append({"app_platform": "android", "url_bin": tmp_url_bin, "url_asc": tmp_url_bin + ".asc", "url_sha256": tmp_url_bin + ".sha256"})
            ...
            downloads.append({"app_platform": "linux", "url_bin": tmp_url_bin, "url_asc": tmp_url_bin + ".asc", "url_sha256": tmp_url_bin + ".sha256"})
